...
import { statusStringToNumber } from "../utils/status";
...
export default function TaskEditForm({ 
  ...
  task: Task; 
  onUpdate: (id: number, title: string, description: string, status: number) => void; 
  onClose: () => void;
}) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const statusNum = statusStringToNumber(task.status);
      await onUpdate(task.id, title.trim(), desc.trim(), statusNum);
      onClose();
    } finally { 
      setSubmitting(false); 
    }
